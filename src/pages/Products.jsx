...
function Products() {
  ...
  if (isBogoShop) {
    ...
    baseProducts = searchApiProducts.length > 0 
      ? searchApiProducts 
      : allProductsList.filter(p => p.name?.toLowerCase().includes(searchQuery.toLowerCase()) || p.brand?.toLowerCase().includes(searchQuery.toLowerCase()));
  } else if (filterType === "sunglasses" || filterType === "eyeglasses") {
    baseProducts = categoryApiProducts.length > 0 
      ? categoryApiProducts 
      : allProductsList.filter(p => p.type === filterType);
  } else {
